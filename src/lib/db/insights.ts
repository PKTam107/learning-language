...
import { dueTodayCount, isNewCard, remainingNew, type QueuePolicy } from "@/lib/queue";
import {
  activeDayCount,
  bestStreak,
  buildSeries,
 ...
 */
export async function fetchProgressData(newPerDay: number): Promise<ProgressData> {
  const sb = supabase();
